              ...
              {data && (
                <div className="flex justify-center">
                  <div className="flex flex-col">
                    ...
                    <Countdown
                      date={getDateObject(data.data.endDate)}
                      renderer={renderer}
                    />
                  </div>
                </div>
      ...
      <div className="bg-white mt-5 mx-auto max-h-full rounded-lg">
        {data && (
          <div className="flex flex-row p-5 gap-10">
            <Avatar
              alt="shop avatar"
              src={data.data.shop?.avatarUrl}
              sx={{ width: 100, height: 100 }}
            />
            <div className="flex flex-col">
              <div className="text-2xl">{data.data.shop?.shopName}</div>
              <div>{"{Sẽ thêm đánh giá ở đây sau}"}</div>
            </div>
          </div>
        )}
      </div>
      {data && (
        <div className="bg-white rounded-lg mt-5 mx-auto max-h-full p-5 mb-5">
          ...
          <div className="grid grid-rows-1">
            <div className="grid grid-cols-12">
              <div className="col-span-2">
                <span className="font-bold text-xl">Tên sản phẩm:</span>
              </div>
          ...
          <div className="grid grid-rows-1">
            <div className="grid grid-cols-12">
              <div className="col-span-2">
                <span className="font-bold text-xl">Xuất xứ:</span>
              </div>
              <div className="col-span-10">
                <span className="text-xl">{data.data.origin}</span>
              </div>
            </div>
          </div>
          <Divider variant="middle" className="pl-0 mt-5 mb-5" />
          <div className="grid grid-rows-1">
            <div className="grid grid-cols-12">
              <div className="col-span-2">
                <span className="font-bold text-xl">Thương hiệu:</span>
              </div>
              <div className="col-span-10">
                <span className="text-xl">{data.data.brand?.name}</span>
              </div>
            </div>
          </div>
          <Divider variant="middle" className="pl-0 mt-5 mb-16" />
          <span className=" text-2xl font-bold mb-10">Mô tả sản phẩm:</span>
          <ShowMoreText
            lines={3}
            more="Xem thêm"
            less="Ẩn bớt"
