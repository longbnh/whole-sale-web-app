...
const provinces = ["Thành phố HCM", "Đồng Nai", "Hà Nội", "Đà Nẵng"];
const district = ["Quận 1", "Quận 2", "Quận 3"];
const ward = ["Phường Tô Ký", "Phường XX", "Phường YY"];
...
const AddressModal = (props: any) => {
  ...
  const handleProvinceChange = (
    event: SelectChangeEvent<typeof provinceName>
  ) => {
    const {
      target: { value },
    } = event;
  ...
  const handleDistrictChange = (
    event: SelectChangeEvent<typeof districtName>
  ) => {
    const {
      target: { value },
    } = event;
